...
class SeatStateTest {
    ...
    void seatState_validName_returnsSeatState() {
        SeatState.seatState("  sold ")
                 ...
                 .onSuccess(state -> assertThat(state).isEqualTo(SeatState.SOLD));
    }

    @Test
    void seatState_unknownName_returnsUnknown() {
        SeatState.seatState("reserved")
                 .onSuccess(state -> fail("Expected unknown state failure"))
                 .onFailure(cause -> assertThat(cause).isInstanceOf(SeatState.Error.Unknown.class));
    }
